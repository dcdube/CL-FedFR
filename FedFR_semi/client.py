...
class Client(object):
    def __init__(self, cid, args, data):
        ...
        log_dir = '/home/master/FedFR-semi/ckpt/FedFR/clients/client_' + str(self.cid)
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        log_file_1 = os.path.join(log_dir, 'semisupervised_statistics.log')
    ...
    def get_pseudo_labeled_data(self,backbone,threshold):
        ...
        self.semilogger.info('# before pseudo correctly labeled data: %d' % (number_of_correct_labels_b))
        self.semilogger.info('# before pseudo incorrectly labeled data: %d' % (number_of_false_labels_b))
        if number_of_correct_labels_b+number_of_false_labels_b != 0:
                ...
                self.semilogger.info("# before pseudolabeling accuracy: %.4f" % accuracy_b)
        else:
            accuracy_b = 0
            self.semilogger.info("# before pseudolabeling accuracy: 0")
        self.semilogger.info('# correctly labeled data: %d' % (number_of_correct_labels))
        self.semilogger.info('# incorrectly labeled data: %d' % (number_of_false_labels))
        if number_of_correct_labels+number_of_false_labels != 0:
            ...
            self.semilogger.info("# Pseudolabeling accuracy: %.4f" % accuracy)
        else:
            accuracy = 0
            self.semilogger.info("# Pseudolabeling accuracy: 0")
