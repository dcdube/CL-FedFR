...
# ----------------------------------------------------------------------------------------------------------------------
# Self Paced Learning (SPL) Implementation
# ----------------------------------------------------------------------------------------------------------------------
#         split_ratio = 0.50 # Ratio of the dataset to be used for training
#         # LOCAL DATASET
#         print('=== Image selection for SPL from the local dataset ===')
#         train_loader = DataLoader(self.train_loader.dataset)
#         print('Length of original local dataset:', len(train_loader))
#
#         loss_values = []  # Initialize loss_values for each ID_base
#         opt_bce = torch.optim.SGD(params=self.bce_module.parameters(), lr=cfg.lr, momentum=0.9,
#                                   weight_decay=cfg.weight_decay)
#         for w in self.bce_module.parameters():
#             opt_bce.state[w] = opt.state[w]
#         pbar = tqdm(total=len(train_loader), ncols=120, leave=True)
#         opt_bce.zero_grad()
...
#             bce_logits, bce_gts = self.bce_module(feature, labels)
#             # Calculate BCE loss for each image in the batch
#             for i in range(len(imgs)):
#                 single_img_bce_loss = 10 * self.bce_loss(bce_logits[i:i + 1], bce_gts[i:i + 1])
#                 loss_values.append(single_img_bce_loss.item())
#             opt_bce.step()
#             pbar.update(1)
#         pbar.close()
#         local_sorted_loss_indices = sorted(range(len(loss_values)), key=lambda k: loss_values[k])
#         local_selected_indices = local_sorted_loss_indices[:int(split_ratio * len(local_sorted_loss_indices))]
#         flattened_local_list = [item for sublist in local_selected_indices for item in sublist]
#
#         # PUBLIC DATASET
#         public_index_count = len(train_loader)
#         public_loss_values = []  # Initialize loss_values for each ID_base
#         public_subset_loader = DataLoader(public_loader_subset.dataset)
#         print('=== Image selection for SPL from the global dataset ===')
#         print('Length of original global dataset:', len(public_subset_loader))
#
#         opt_bce = torch.optim.SGD(params=self.bce_module.parameters(), lr=cfg.lr, momentum=0.9,
#                                   weight_decay=cfg.weight_decay)
#         for w in self.bce_module.parameters():
#             opt_bce.state[w] = opt.state[w]
#         pbar = tqdm(total=len(public_subset_loader), ncols=120, leave=True)
#         for step, (imgs, labels) in enumerate(public_subset_loader):
#             opt_bce.zero_grad()
#             imgs = imgs.to(self.local_rank)
#             labels = labels.to(self.local_rank)
#             with torch.no_grad():
...
#             for i in range(len(imgs)):
#                 single_img_bce_loss = 10 * self.bce_loss(bce_logits[i:i + 1], bce_gts[i:i + 1])
#                 public_loss_values.append(single_img_bce_loss.item())
#             opt_bce.step()
#             pbar.update(1)
#         pbar.close()
#         # Select % of images with the smallest loss values
#         public_sorted_loss_indices = sorted(range(len(public_loss_values)), key=lambda k: public_loss_values[k])
#         for i in range(len(public_sorted_loss_indices)):
#             public_sorted_loss_indices[i] += public_index_count
#         public_selected_indices = public_sorted_loss_indices[:int(split_ratio * len(public_sorted_loss_indices))]
#
#         # Indices of selected images from local and global dataset
#         selected_indices = flattened_local_list + public_selected_indices
#
#         selected_subset  = Subset(combine_dataset, selected_indices)
#         print('Length of selected dataset for SPL:', len(selected_subset))
#
#         # Create a new DataLoader with only selected images
#         combine_loader = DataLoader(selected_subset , batch_size=cfg.com_batch_size, shuffle=True, num_workers=6,
#                                     pin_memory=True, drop_last=True)

# # ----------------------------------------------------------------------------------------------------------------------
# # Self Paced Learning with Diversity (SPLD) Implementation
# # ----------------------------------------------------------------------------------------------------------------------
#         split_ratio = 1.0 # Ratio of the dataset to be used for training
#         # LOCAL DATASET
#         print('=== Image selection for SPLD from the local dataset ===')
#         train_loader = DataLoader(self.train_loader.dataset)
#         print('Length of original local dataset:', len(train_loader))
#
#         local_selected_indices_list = []
#         index_count = 0
#         no_IDs = 100
#
#         pbar = tqdm(total=len(train_loader), ncols=120, leave=True)
#         for target_id in range(no_IDs):
#             # Filter the train_loader dataset based on the target ID
#             filtered_dataset = Subset(self.train_loader.dataset,
#                                       [i for i, (_, label) in enumerate(self.train_loader.dataset) if
#                                        label == target_id])
#
#             # Create a DataLoader for the filtered dataset
#             train_loader_for_ID = DataLoader(filtered_dataset)
#
#             loss_values = []  # Initialize loss_values for each ID_base
#             for step, (imgs, labels) in enumerate(train_loader_for_ID):
#                 imgs = imgs.to(self.local_rank)
#                 labels = labels.to(self.local_rank)
#                 with torch.no_grad():
#                     feature = backbone(imgs)
...
#             else:
#                 local_sorted_loss_indices = sorted(range(len(loss_values)), key=lambda k: loss_values[k])
#                 index_count += len(local_sorted_loss_indices)
#                 local_selected_indices = [index + index_count for index in
#                                           local_sorted_loss_indices[:int(split_ratio * len(local_sorted_loss_indices))]]
#             # Save local_selected_indices for the current target_id
#             local_selected_indices_list.append(local_selected_indices)
#         pbar.close()
#         flattened_local_list = [item for sublist in local_selected_indices_list for item in sublist]
#
#         # # CRFIQA for LOCAL ---------------------------------------------------------------------------------------------
#         CRFIQA_local_path = "/home/master/FedFR-main/ms1m_split/split_train_i4000c0040/CRFIQA_local.csv"
#         CRFIQA_local_df = pd.read_csv(CRFIQA_local_path)
#         # Select the crfiqa_score corresponding to the indices in flattened_local_list
#         selected_scores = CRFIQA_local_df.loc[flattened_local_list, "crfiqa_score"]
#         flattened_local_list = selected_scores.sort_values(ascending=False).index.tolist()
#         print("Local images ordered using CR-FIQA")
#         # # CRFIQA for LOCAL ---------------------------------------------------------------------------------------------
#
#         # PUBLIC DATASET
#         public_index_count = len(train_loader)
#         public_loss_values = []  # Initialize loss_values for each ID_base
#         public_subset_loader = DataLoader(public_loader_subset.dataset)
#         print('=== Image selection for SPLD from the global dataset ===')
#         print('Length of original global dataset:', len(public_subset_loader))
#
...
#
#         # # CRFIQA for GLOBAL --------------------------------------------------------------------------------------------
#         CRFIQA_global_path = "/home/master/FedFR-main/ms1m_split/split_pretrain_6000/CRFIQA_global.csv"
#         CRFIQA_global_df = pd.read_csv(CRFIQA_global_path)
#         public_scores = CRFIQA_global_df.loc[public_selected_indices, "crfiqa_score"]
#         public_selected_indices = public_scores.sort_values(ascending=False).index.tolist()
#         print("Global images ordered using CR-FIQA")
#         # # CRFIQA for GLOBAL --------------------------------------------------------------------------------------------
#
#         # Indices of selected images from local and global dataset
#         selected_indices = flattened_local_list + public_selected_indices
#
#         selected_subset  = Subset(combine_dataset, selected_indices)
#         print('Length of selected dataset for SPLD:', len(selected_subset))
#
#         # Create a new DataLoader with only selected images
#         combine_loader = DataLoader(selected_subset , batch_size=cfg.com_batch_size, shuffle=True, num_workers=6,
#                                     pin_memory=True, drop_last=True)
# ----------------------------------------------------------------------------------------------------------------------
# CL-FedFR Based on CR-FIQA Implementation
# ----------------------------------------------------------------------------------------------------------------------
#         split_by = 2 # 2 for 50% split
#
#         # CRFIQA for LOCAL ---------------------------------------------------------------------------------------------
#         train_loader = DataLoader(self.train_loader.dataset)
#         print('Length of original local dataset:', len(train_loader))
#
#         local_labels = []
#         for _, (_, label) in enumerate(self.train_loader.dataset):
#             local_labels.append(label.item())  # Convert tensor to a Python scalar
#
#         flattened_local_list = local_labels
#
#         CRFIQA_local_path = "/home/master/FedFR-main/ms1m_split/split_train_i4000c0040/CRFIQA_local.csv"
#         CRFIQA_local_df = pd.read_csv(CRFIQA_local_path)
#
#         # Select the crfiqa_score corresponding to the indices in flattened_local_list
#         local_scores = CRFIQA_local_df.loc[flattened_local_list, "crfiqa_score"]
#         local_sorted_indices = local_scores.sort_values(ascending=False).index.tolist()
#
#         # Split local dataset
#         local_split = len(local_sorted_indices) // split_by
#         flattened_local_list = local_sorted_indices[:local_split]
#         print("Number of local images selected using CR-FIQA:", len(flattened_local_list))
#
#         # CRFIQA for GLOBAL --------------------------------------------------------------------------------------------
#         public_subset_loader = DataLoader(public_loader_subset.dataset)
#         print('Length of original global dataset:', len(public_subset_loader))
#
#         public_index_count = len(train_loader) # To shift the global indices
#         public_indices = []
#
#         for _, (_, label) in enumerate(public_loader_subset.dataset):
#             shifted_label = label.item() + public_index_count
#             public_indices.append(shifted_label)
#
#         CRFIQA_global_path = "/home/master/FedFR-main/ms1m_split/split_pretrain_6000/CRFIQA_global.csv"
#         CRFIQA_global_df = pd.read_csv(CRFIQA_global_path)
#         public_scores = CRFIQA_global_df.loc[public_indices, "crfiqa_score"]
#         public_sorted_indices = public_scores.sort_values(ascending=False).index.tolist()
#
#         global_split = len(public_sorted_indices) // split_by
#         public_selected_indices = public_sorted_indices[:global_split]
#         print("Number of global images selected using CR-FIQA:", len(public_selected_indices))
#
#         # Indices of selected images from local and global dataset
#         selected_indices = flattened_local_list + public_selected_indices
#
#         selected_subset = Subset(combine_dataset, selected_indices)
#         print('Length of selected dataset for CR-FIQA training:', len(selected_subset))
#
#         # Create a new DataLoader with only selected images
#         combine_loader = DataLoader(selected_subset, batch_size=cfg.com_batch_size, shuffle=True, num_workers=6,
#                                     pin_memory=True, drop_last=True)
...
# ----------------------------------------------------------------------------------------------------------------------
        for epoch in range(start_epoch, start_epoch + self.local_epoch):
            self.logger.info(
                'Epoch %d, Total Epoch %d, Total step : %d, lr=%.4f' % (epoch, start_epoch + self.local_epoch,
            ...
            if self.args.BCE_local:  ### train with BCE loss
                for step, (imgs, labels) in enumerate(combine_loader):
                    ...
                    if self.args.contrastive_bb:
                        ...
                        cos_logits, bce_logits, bce_gts, feats = model(imgs, labels, contrastive=True,
                                                                       detach=self.args.BCE_detach)
                        # Contrastive
                        ...
                        cos_logits, bce_logits, bce_gts = model(imgs, labels, contrastive=False,
                                                                detach=self.args.BCE_detach)
                        # Cosface
                        cos_logits = self.margin_softmax(cos_logits, labels)
                        if self.args.reweight_cosface:
                    ...
                    if step > 10 and step % 40 == 0:
                        pbar.set_postfix(
                            loss='%.3f,%.3f,%.3f,%.3f' % (loss_meter.avg, cos_meter.avg, con_meter.avg, bce_meter.avg))
                        self.logger.debug('Step %d, Loss : %.3f,%.3f,%.3f,%.3f' % (
                        step, loss_meter.avg, cos_meter.avg, con_meter.avg, bce_meter.avg))
                    pbar.update(1)
            else:
                for step, (imgs, labels) in enumerate(combine_loader):
                    opt.zero_grad()
                    ...
                    if step > 10 and step % 40 == 0:
                        ...
                        self.logger.debug(
                            'Step %d, Loss : %.3f,%.3f,%.3f' % (step, loss_meter.avg, cos_meter.avg, con_meter.avg))
                    pbar.update(1)
            pbar.close()
            schler.step()

        if self.rank == 0:
            self.logger.info("Client %d Ends: loss = %.3f" % (self.cid, loss_meter.avg))
