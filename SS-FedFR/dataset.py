...
class All_Client_Dataset(object):
    def __init__(self,root_dir,local_rank,args):
        ...
        self.creating_each_client(self.args.split_ratio)
    ...
    def creating_each_client(self,split_ratio=1.0):
        self.train_loaders = []
        self.train_unlabeled_loaders = [] ##****
        self.train_dataset_sizes = []
        self.train_unlabeled_dataset_sizes = []
        self.train_class_sizes = []
        self.train_unlabeled_class_sizes = []
        for c in range(self.num_client):
            client_dir = osp.join(self.dataset_dir,'client_%04d'%(c))
            dataset = MXFaceDataset_Split(root_dir=client_dir,local_rank=self.local_rank,transform=self.transform,split_ratio=split_ratio)
            unlabeled_dataset = MXFaceDataset_Split(root_dir=client_dir,local_rank=self.local_rank,transform=self.transform,split_ratio=split_ratio,labeled=False)
            train_loader = DataLoader(dataset,batch_size=self.batch_size,shuffle=True,num_workers=2,pin_memory=True,drop_last=False)
            train_unlabeled_loader =  DataLoader(unlabeled_dataset, batch_size=self.batch_size,shuffle=True,num_workers=2,pin_memory=True,drop_last=False)
            self.train_loaders.append(train_loader)
            self.train_unlabeled_loaders.append(train_unlabeled_loader)
            self.train_unlabeled_dataset_sizes.append(len(unlabeled_dataset))
            self.train_dataset_sizes.append(len(dataset))
            self.train_class_sizes.append(dataset.num_classes)
            self.train_unlabeled_class_sizes.append(unlabeled_dataset.num_classes)

        logger.info('--------------------')
        logger.info('Num_clients : %d'%self.num_client)
        logger.info('Train dataset class sizes /client: %d'%self.train_class_sizes[1])
...
class MXFaceDataset_Combine(Dataset):
    def __init__(self, first_dataset, second_dataset, third_dataset):
        ...
        self.second_nclass = second_dataset.num_classes
        self.third_nclass = third_dataset.num_classes

        self.first_len = len(first_dataset)
        self.second_len = len(second_dataset)
        self.third_len = len(third_dataset)

        self.num_class = self.first_nclass + self.second_nclass + self.third_nclass

    def __getitem__(self, idx):
        if idx < self.first_len:
            img, label = self.first_dataset[idx]
            return img, label
        elif idx < self.first_len + self.second_len:
            img, label = self.second_dataset[idx - self.first_len]
            return img, label
        else:
            img, label = self.third_dataset[idx - self.first_len - self.second_len]
            return img, label + self.first_nclass

    def __len__(self):
        return self.first_len + self.second_len + self.third_len


class MXFaceDataset_Split(Dataset):
    def __init__(self, root_dir, local_rank,transform,split_ratio=1.0,labeled=True):
        ...
        np.random.shuffle(self.imgidx) #bozuyor olabilir
        split_index = int(len(self.imgidx) * split_ratio)
        if labeled:
            self.imgidx = self.imgidx[:split_index]
        else:
            self.imgidx = self.imgidx[split_index:]
        self.transform = transform
...
class MXFaceDataset_Split_PseudoLabeled(Dataset):
    def __init__(self, data_tensor, label_tensor):
        super(MXFaceDataset_Split_PseudoLabeled, self).__init__()
...
if __name__ == '__main__':
    ...
    dataset = MXFaceDataset_Split(root_dir='/home/master/SS-FedFR/ms1m_split/split_0.20_c0001/client_0000',local_rank=0,transform=None)
    print(dataset.num_classes)
    exit(-1)
    for i in range(1000):
        dataset = MXFaceDataset(root_dir='/home/master/SS-FedFR/ms1m_split/split_1000.00_c1000/client_%04d'%i,local_rank=0,transform=None)
        # dataset = MXFaceDataset(root_dir='/home/master/SS-FedFR/faces_glintasia/',local_rank=0)
